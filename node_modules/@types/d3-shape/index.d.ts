...
export interface DefaultArcObject {
    ...
    outerRadius: number;
    /**
     * Start angle of arc. The angle is specified in radians, with 0 at -y (12 o?셛lock) and positive angles proceeding clockwise.
     */
    startAngle: number;
    /**
     * End angle of arc. The angle is specified in radians, with 0 at -y (12 o?셛lock) and positive angles proceeding clockwise.
     */
    endAngle: number;
    /**
 ...
 *
 * If the difference between the start and end angles (the angular span) is greater than ?, the arc generator will produce a complete circle or annulus.
 * If it is less than ?, arcs may have rounded corners and angular padding. Arcs are always centered at ??,0?? use a transform (see: SVG, Canvas) to move the arc to a different position.
 *
...
export interface Arc<This, Datum> {
     ...
     * In addition, for arcs whose angular span is less than ?, the corner radius may be reduced as two adjacent rounded corners intersect.
     ...
     *
     * The angle is specified in radians, with 0 at -y (12 o?셛lock) and positive angles proceeding clockwise.
     * If |endAngle - startAngle| ???, a complete circle or annulus is generated rather than a sector.
     *
     ...
     * If the arc forms a complete circle or annulus, as when |endAngle - startAngle| ???, the pad angle is ignored. If the inner radius or angular span is small relative to the pad angle,
     ...
     *
     * The recommended minimum inner radius when using padding is outerRadius * padAngle / sin(罐), where 罐 is the angular span of the smallest arc before padding.
     * For example, if the outer radius is 200 pixels and the pad angle is 0.02 radians, a reasonable 罐 is 0.04 radians, and a reasonable inner radius is 100 pixels.
     *
...
export interface PieArcDatum<T> {
     ...
     * then the units are in radians with 0 at -y (12 o?셛lock) and positive angles proceeding clockwise.
     */
    startAngle: number;
    /**
...
export interface Pie<This, Datum> {
    /**
     * Generates a pie for the given array of data, returning an array of objects representing each datum?셲 arc angles.
     * Any additional arguments are arbitrary; they are simply propagated to the pie generator?셲 accessor functions along with the this object.
     ...
     *
     * This representation is designed to work with the arc generator?셲 default startAngle, endAngle and padAngle accessors.
     * The angular units are arbitrary, but if you plan to use the pie generator in conjunction with an arc generator,
     * you should specify angles in radians, with 0 at -y (12 o?셛lock) and positive angles proceeding clockwise.
     *
     ...
     * you should specify an angle in radians, with 0 at -y (12 o?셛lock) and positive angles proceeding clockwise.
     *
     ...
     * you should specify an angle in radians, with 0 at -y (12 o?셛lock) and positive angles proceeding clockwise.
     *
     * The value of the end angle is constrained to startAngle 짹 ?, such that |endAngle - startAngle| ???.
     *
     * @param angle A constant angle.
     */
    endAngle(angle: number): this;
...
export interface Line<Datum> {
    /**
     * Generates a line for the given array of data. Depending on this line generator?셲 associated curve,
 ...
 * Radial lines are always positioned relative to ??,0?? use a transform (see: SVG, Canvas) to change the origin.
 *
 * The generic refers to the data type of an element in the input array passed into the line generator.
 */
export interface LineRadial<Datum> {
    /**
     * Generates a radial line for the given array of data. Depending on this radial line generator?셲 associated curve,
     * the given input data may need to be sorted by x-value before being passed to the line generator.
     *
     ...
     *
     * @param angle A constant angle value in radians, with 0 at -y (12 o?셛lock).
     */
     ...
     * then you should specify a custom accessor.
     *
     * @param angle An angle accessor function which returns the angle value in radians, with 0 at -y (12 o?셛lock). The angle accessor will be invoked for each defined element in the input data array,
     * being passed the element d, the index i, and the array data as three arguments.
     */
...
export interface Area<Datum> {
    /**
     * Generates an area for the given array of data. Depending on this area generator?셲 associated curve,
    ...
    /**
     * Returns a new line generator that has this area generator?셲 current defined accessor, curve and context.
     * The line?셲 x-accessor is this area?셲 x0-accessor, and the line?셲 y-accessor is this area?셲 y0-accessor.
     */
    lineX0(): Line<Datum>;
    /**
     * Returns a new line generator that has this area generator?셲 current defined accessor, curve and context.
    ...
    /**
     * Returns a new line generator that has this area generator?셲 current defined accessor, curve and context.
     * The line?셲 x-accessor is this area?셲 x1-accessor, and the line?셲 y-accessor is this area?셲 y0-accessor.
     */
    lineX1(): Line<Datum>;
    /**
     * Returns a new line generator that has this area generator?셲 current defined accessor, curve and context.
     * The line?셲 x-accessor is this area?셲 x0-accessor, and the line?셲 y-accessor is this area?셲 y1-accessor.
     */
    lineY1(): Line<Datum>;
}
 ...
 * except the x and y accessors are replaced with angle and radius accessors.
 * Radial areas are always positioned relative to ??,0?? use a transform (see: SVG, Canvas) to change the origin.
 *
 * The generic refers to the data type of an element in the input array passed into the area generator.
 */
export interface AreaRadial<Datum> {
     ...
     *
     * @param angle A constant value in radians with 0 at -y (12 o?셛lock).
     */
    angle(angle: number): this;
    /**
     ...
     * If your data are in a different format, or if you wish to transform the data before rendering, then you should specify a custom accessor.
     *
     * @param angle An accessor function returning a value to be used for startAngle in radians with 0 at -y (12 o?셛lock).
    ...
    /**
     * Equivalent to area.x1, except the accessor returns the angle in radians, with 0 at -y (12 o?셛lock).
     ...
     * If your data are in a different format, or if you wish to transform the data before rendering, then you should specify a custom accessor.
     *
     * @param angle An accessor function returning a value to be used for endAngle in radians with 0 at -y (12 o?셛lock).
     * The accessor will be invoked for each defined element in the input data array,
     * being passed the element d, the index i, and the array data as three arguments.
     */
    ...
    /**
     * Equivalent to area.y1, except the accessor returns the radius: the distance from the origin ??,0??
     */
    ...
    /**
     * Returns a new radial line generator that has this radial area generator?셲 current defined accessor, curve and context.
     * The line?셲 angle accessor is this area?셲 start angle accessor, and the line?셲 radius accessor is this area?셲 inner radius accessor.
     */
    ...
    /**
     * Returns a new radial line generator that has this radial area generator?셲 current defined accessor, curve and context.
     * The line?셲 angle accessor is this area?셲 end angle accessor, and the line?셲 radius accessor is this area?셲 inner radius accessor.
     */
    lineEndAngle(): LineRadial<Datum>;

    /**
     * Returns a new radial line generator that has this radial area generator?셲 current defined accessor, curve and context.
     * The line?셲 angle accessor is this area?셲 start angle accessor, and the line?셲 radius accessor is this area?셲 outer radius accessor.
     */
...
/**
 * Produces a B챕zier curve between each pair of points, with horizontal tangents at each point.
 */
export const curveBumpX: CurveFactory;

/**
 * Produces a B챕zier curve between each pair of points, with vertical tangents at each point.
 */
 ...
 * with the spline straightened according to the curve?셲 beta, which defaults to 0.85.
...
export interface CurveCardinalFactory extends CurveFactory {
     ...
     * The tension determines the length of the tangents: a tension of one yields all zero tangents, equivalent to curveLinear; a tension of zero produces a uniform Catmull?밨om spline.
     *
...
/**
 * A curve factory for cubic Catmull?밨om spline generators.
 */
export interface CurveCatmullRomFactory extends CurveFactory {
    /**
     * Returns a cubic Catmull?밨om curve factory with the specified alpha in the range [0, 1].
 ...
 * A curve factory for cubic Catmull?밨om spline generators.
 *
 * The curve generators produce a cubic Catmull?밨om spline using the specified control points and the parameter alpha,
 * which defaults to 0.5, as proposed by Yuksel et al. in On the Parameterization of Catmull?밨om Curves,
 ...
 * A curve factory for cubic Catmull?밨om spline generators.
 *
 * The curve generators produce a closed cubic Catmull?밨om spline using the specified control points and the parameter alpha,
 ...
 * ?쏿 smooth curve with continuous first-order derivatives that passes through any given set of data points without spurious oscillations.
...
export interface DefaultLinkObject {
     ...
     * the coordinates [angle, radius]. The angle is stated in radians, with 0 at -y (12 o?셛lock).
     * The radius measures the distance from the origin ??,0??
     */
...
/**
 * A link generator for a Cartesian coordinate system. The link shape generates a smooth cubic B챕zier curve from a
...
export interface LinkRadial<This, LinkDatum, NodeDatum> {
     ...
     * The angle is stated in radians, with 0 at -y (12 o?셛lock).
     *
     ...
     * The radius is measured as the distance from the origin ??,0??
     *
 ...
 * A symbol generator.
 *
 * Symbols provide a categorical shape encoding as is commonly used in scatterplots. Symbols are always centered at ??,0??
...
export interface Symbol<This, Datum> {
     ...
     * If you wish to scale the symbol to fit a given bounding box, rather than by area, try SVG?셲 getBBox.
     *
 ...
 * @param angle Angle in radians, with 0 at -y (12 o?셛lock) and positive angles proceeding clockwise.
 * @param radius Radius.
 */
...
export interface Stack<This, Datum, Key> {
     ...
     *
     * Any additional arguments are arbitrary; they are simply propagated to the generator?셲 accessor functions along with the this object.
     *
     * @param data Array of data elements.
     */
     ...
     * The series??key is passed to the value accessor, along with each data point, to compute the point?셲 value.
     */
 ...
 * This order is recommended for streamgraphs in conjunction with the wiggle offset. See Stacked Graphs?봆eometry & Aesthetics by Byron & Wattenberg for more information.
 *
 * @param series A series generated by a stack generator.
 */
 ...
 * See Stacked Graphs?봆eometry & Aesthetics by Bryon & Wattenberg for more information.
 *
 * @param series A series generated by a stack generator.
 * @param order An array of numeric indexes representing the stack order.
 */
