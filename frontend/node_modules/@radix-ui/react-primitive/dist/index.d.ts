 ...
 * For example:
 *
 * dispatching a known click ?몠
 * target.dispatchEvent(new Event(?쁟lick??)
 *
 * dispatching a custom type within a non-discrete event ?몠
 * onScroll={(event) => event.target.dispatchEvent(new CustomEvent(?쁟ustomType??)}
 *
 * dispatching a custom type within a `discrete` event ?몟
 * onPointerDown={(event) => dispatchDiscreteCustomEvent(event.target, new CustomEvent(?쁟ustomType??)}
 *
